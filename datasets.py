...
class TemporalDataset(object):
    ...
    def eval(
        ...
        for m in missing:
            ...
            #att_weights = model.get_attention_weights(q, batch_size=500)
    ...
    def eval_attention(
        ...
        for m in missing:
            ...
            if m == 'lhs':
                ...
                q[:, 2] = tmp
                q[:, 1] += self.n_predicates // 2
            att_weights[m] = model.get_attention_weights(q, batch_size=500)
        att_weights = [(att_weights['lhs'][i] + att_weights['rhs'][i]) for i in range(4)]

        return att_weights

    def eval_instance(
            self, model: TKBCModel, data, n_queries: int = -1, missing_eval: str = 'both',
            at: Tuple[int] = (1, 3, 10)
    ):
        if self.events is not None:
            return self.time_eval_instance(model, data, n_queries, 'rhs', at)
        examples = torch.from_numpy(data.astype('int64')).to('cuda' if torch.cuda.is_available() else 'cpu')
        missing = [missing_eval]
        if missing_eval == 'both':
            missing = ['rhs', 'lhs']
